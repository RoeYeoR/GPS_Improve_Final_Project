...
public class LosNlosComputer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<Point3D> routePoints = readRoutePoints();  // קריאת נקודות המסלול
    ...
    private static List<Point3D> readRoutePoints() throws Exception {
        List<Point3D> points = new ArrayList<>();
        DocumentBuilderFactory dbFactory = DocumentBuilderFactory.newInstance();
        DocumentBuilder dBuilder = dbFactory.newDocumentBuilder();
        Document doc = dBuilder.parse(new File("routeABCDFabricated.kml"));
    ...
    private static void generateKMLWithLines(List<Point3D> processedPoints, List<Sat> satellites, List<Building> buildings, String filename) {
        try {
            ...
            Point3D selectedPoint = findPointWith4LOS4NLOS(buildings, satellitePositions);
    ...
    private static Point3D findPointWith4LOS4NLOS(List<Building> buildings, Map<String, Point3D> satellitePositions) {
        try {
            List<Point3D> routePoints = readRoutePoints();
            for (Point3D point : routePoints) {
                ...
                if (losCount == 4 && nlosCount == 4) {
                    System.out.println("Found point with 4 LOS and 4 NLOS satellites at index " + routePoints.indexOf(point));
                    return point;
                }
            }
            System.out.println("Could not find a point with exactly 4 LOS and 4 NLOS satellites");
            return routePoints.get(0); // Return first point if no suitable point found
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
