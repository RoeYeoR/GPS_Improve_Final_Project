...
public class LosNlosComputer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < routePoints.size(); i++) {
            ...
            // Compute LOS/NLOS for current point
            List<Boolean> losResults = computeLosNlos(satellites, buildings, currentPointUTM);
            
            // Count LOS and NLOS satellites
            int losCount = 0;
            for (Boolean isLos : losResults) {
                if (isLos) losCount++;
            }
            int nlosCount = losResults.size() - losCount;
            
            // Check if this point has exactly 6 LOS and 3 NLOS satellites
            if (losCount == 6 && nlosCount == 3) {
                System.out.println("\nFound point with 6 LOS and 3 NLOS satellites!");
                System.out.println("Point coordinates: " + currentPoint);
                ...
                try (FileWriter writer = new FileWriter(outputKMLPath)) {
                    ...
                    writer.write("      <n>Found Point (6 LOS, 3 NLOS)<n>\n");
                    writer.write("      <Point>\n");
                    ...
                    for (int j = 0; j < satellites.size(); j++) {
                        ...
                        writer.write("    <Placemark>\n");
                        writer.write("      <n>Satellite " + (j + 1) + " - " + (isLos ? "LOS" : "NLOS") + "<n>\n");
                ...
                // We found what we were looking for, stop processing more points
                break;
            }
            ...
            losResults = computeLosNlos(satellites, buildings, currentPointUTM);
        ...
        for (int i = 0; i < processedPoints.size(); i++) {
            ...
            if (losCount == 3 && losResults.size() - losCount == 4) {
                selectedPoint = point;
