    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < routePoints.size(); i++) {
            ...
            if (losCount == 6 && nlosCount == 2) {
                ...
                // Write to KML file only for the first matching point
                if (!foundFirstPoint) {
                    foundFirstPoint = true;
                    String outputKMLPath = "c:/Users/A/Desktop/לימודים/FinalProjectGPS/Final_project-main-master/found_point.kml";
                    try (FileWriter writer = new FileWriter(outputKMLPath)) {
                        ...
                        writer.write("      <description>Analysis Point Location</description>\n");
                        writer.write("      <Point>\n");
                        writer.write("        <altitudeMode>absolute</altitudeMode>\n");
                        writer.write("        <coordinates>" + currentPoint.getY() + "," + currentPoint.getX() + "," + currentPoint.getZ() + "</coordinates>\n");
                        writer.write("      </Point>\n");
                        writer.write("    </Placemark>\n");
                        
                        // Write lines to satellites
                        for (int j = 0; j < satellites.size(); j++) {
                            boolean isLos = losResults.get(j);
                            Point3D satPoint = satellites.get(j).getSatPosInECEF();
                            
                            writer.write("    <Placemark>\n");
                            writer.write("      <name>Satellite " + (j + 1) + " - " + (isLos ? "LOS" : "NLOS") + "</name>\n");
                            writer.write("      <description>Elevation: " + String.format("%.1f", satellites.get(j).getElevetion()) + 
                                       "°, Azimuth: " + String.format("%.1f", satellites.get(j).getAzimuth()) + "°</description>\n");
                            writer.write("      <styleUrl>#" + (isLos ? "losStyle" : "nlosStyle") + "</styleUrl>\n");
                            writer.write("      <LineString>\n");
                            writer.write("        <tessellate>1</tessellate>\n");
                            writer.write("        <altitudeMode>absolute</altitudeMode>\n");
                            writer.write("        <coordinates>\n");
                            writer.write("          " + currentPoint.getY() + "," + currentPoint.getX() + "," + currentPoint.getZ() + "\n");
                            ...
                            // Add satellite point
                            writer.write("    <Placemark>\n");
                            writer.write("      <name>Satellite " + (j + 1) + " Position</name>\n");
                            writer.write("      <styleUrl>#" + (isLos ? "losStyle" : "nlosStyle") + "</styleUrl>\n");
                            writer.write("      <Point>\n");
                            writer.write("        <altitudeMode>absolute</altitudeMode>\n");
                            writer.write("        <coordinates>" + satPoint.getX() + "," + satPoint.getY() + "," + satPoint.getZ() + "</coordinates>\n");
                            writer.write("      </Point>\n");
                            writer.write("    </Placemark>\n");
                        }
                        ...
                        System.out.println("\nKML file generated: " + outputKMLPath);
                    } catch (IOException e) {
                        System.err.println("Error writing KML file: " + e.getMessage());
                    }
                } else {
                    System.out.println("Found another matching point, but KML already generated for first point.");
                }
        ...
        for (int i = 0; i < processedPoints.size(); i++) {
            ...
            if (losCount == 6 && losResults.size() - losCount == 2) {
                selectedPoint = point;
        ...
        if (selectedPoint != null) {
            ...
            try (FileWriter writer = new FileWriter(visualizationKML)) {
                ...
                writer.write("      <Point>\n");
                writer.write("        <altitudeMode>absolute</altitudeMode>\n");
                writer.write("        <coordinates>" + selectedPoint.getX() + "," + selectedPoint.getY() + "," + selectedPoint.getZ() + "</coordinates>\n");
                writer.write("      </Point>\n");
                writer.write("    </Placemark>\n");
                ...
                for (int i = 0; i < satellites.size(); i++) {
                    if (i < selectedLosResults.size()) {
                        ...
                        writer.write("        <altitudeMode>absolute</altitudeMode>\n");
                        writer.write("        <coordinates>\n");
                        writer.write("          " + selectedPoint.getX() + "," + selectedPoint.getY() + "," + selectedPoint.getZ() + "\n");
                        writer.write("          " + satPoint.getX() + "," + satPoint.getY() + "," + satPoint.getZ() + "\n");
                        writer.write("        </coordinates>\n");
                        writer.write("      </LineString>\n");
                        writer.write("    </Placemark>\n");
